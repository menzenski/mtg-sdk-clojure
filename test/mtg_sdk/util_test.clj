...
(deftest parse-data-test
  (testing "json response is parsed with keys as keywords"
    (let [raw-str (str "{\"types\":[\"Artifact\",\"Conspiracy\",\"Creature\","
                       ...
                       "\"Tribal\",\"Vanguard\"]}")]
      (is (true? (contains? (util/parse-data raw-str) :types))))
    (let [raw-str (str "{\"set\":{\"code\":\"KTK\",\"name\":\"Khans of "
      ...
      (is (true? (contains? (util/parse-data raw-str) :set)))
      (let [parsed (util/parse-data raw-str)]
...
(deftest build-query-string-test
  (testing "a single string value is returned without modification"
    (is (= (util/build-query-string "KLD") "KLD")))
  (testing "multiple string values are joined by default pipe separator"
    (is (= (util/build-query-string "KLD" "AER") "KLD|AER"))))
